...
export default function PrintPage(){
 ...
 const [snapshot,setSnapshot]=useState(''),[preparing,setPreparing]=useState(false),[printStatus,setPrintStatus]=useState('')
 ...
 useEffect(()=>{
  ...
  const map=L.map(mapNode.current,{center:campus.ui?.map?.center||[64.857,-147.829],zoom:Number(campus.ui?.map?.zoom||16),zoomControl:true,attributionControl:true,keyboard:true,preferCanvas:false})
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:20,attribution:'© OpenStreetMap contributors',crossOrigin:'anonymous'}).addTo(map)
 ...
 useEffect(()=>{if(mode==='full'&&mapRef.current){mapRef.current.fitBounds(boundsForCampus(),{padding:[28,28],maxZoom:16});setTimeout(()=>mapRef.current?.invalidateSize(),80)}},[mode,paper])
 useEffect(()=>{const clear=()=>{setSnapshot('');setPrintStatus('')};window.addEventListener('afterprint',clear);return()=>window.removeEventListener('afterprint',clear)},[])
 ...
 function resetFull(){setMode('full');setSnapshot('');updateParam('mode','full');mapRef.current?.fitBounds(boundsForCampus(),{padding:[28,28],maxZoom:16});setTimeout(()=>mapRef.current?.invalidateSize(),80)}

 async function waitForTiles(){
  const root=mapNode.current;if(!root)return
  const images=[...root.querySelectorAll('.leaflet-tile')]
  const pending=images.filter(img=>!img.complete)
  if(!pending.length){await wait(80);return}
  await Promise.race([
   Promise.all(pending.map(img=>new Promise(resolve=>{const done=()=>resolve();img.addEventListener('load',done,{once:true});img.addEventListener('error',done,{once:true})}))),
   wait(3000)
  ])
  await wait(120)
 }
 ...
 async function printNow(){
  if(preparing||!mapNode.current)return
  setPreparing(true);setPrintStatus('Preparing print-quality map…')
  try{
   mapRef.current?.invalidateSize()
   await document.fonts?.ready
   await waitForTiles()
   const canvas=await html2canvas(mapNode.current,{useCORS:true,allowTaint:false,backgroundColor:'#e7edf0',scale:Math.min(2,window.devicePixelRatio||1.5),logging:false,imageTimeout:5000,removeContainer:true})
   const url=canvas.toDataURL('image/png',1)
   setSnapshot(url);setPrintStatus('Opening Print / Save…')
   await wait(160)
   window.print()
  }catch(err){
   console.error(err)
   setPrintStatus('Could not prepare the map for printing. Reload the print page and try again.')
 ...
 return <main id="main-content" className={`print-page true-print ${paper} ${mode}`}>
  <div className="print-tools" aria-label="Print map controls">
   ...
   <label className="print-toggle"><input type="checkbox" checked={showBuildings} onChange={e=>{setShowBuildings(e.target.checked);setSnapshot('')}}/> Building names</label>
   <label className="print-toggle"><input type="checkbox" checked={showParking} onChange={e=>{setShowParking(e.target.checked);setSnapshot('')}}/> Parking</label>
   <label className="print-toggle"><input type="checkbox" checked={showShapes} onChange={e=>{setShowShapes(e.target.checked);setSnapshot('')}}/> Map shapes</label>
   <button type="button" onClick={resetFull}>Show full campus</button>
   <button type="button" className="print-button" disabled={preparing} onClick={printNow}>{preparing?'Preparing…':'Print / Save'}</button>
   <span className="print-status" role="status" aria-live="polite">{printStatus}</span>
  </div>
  <section className="print-live-shell">
   ...
   <div className="print-map-stage">
    ...
    {snapshot&&<img className="print-map-snapshot" src={snapshot} alt="Print-ready snapshot of the selected UAF campus map view"/>}
   </div>
