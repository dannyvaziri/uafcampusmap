...
$root = __DIR__;
$buildings = [];
foreach (glob($root . '/src/data/buildings-*.json') ?: [] as $file) $buildings = array_merge($buildings, read_json($file, []));
$parking = [];
foreach (glob($root . '/src/data/parking-*.json') ?: [] as $file) $parking = array_merge($parking, read_json($file, []));
$meta = read_json($root . '/src/data/meta.json', []);
$config = read_json($root . '/public/map-config.json', []);
...
<header class="site-header"><a class="brand" href="/"><img src="/public/uaf-logo.svg" alt="University of Alaska Fairbanks"><span>Campus Map</span></a><nav><a href="/">Map</a><a href="/accessible">Text map</a><a href="/print">Print</a><a href="/admin">Admin</a></nav></header>
