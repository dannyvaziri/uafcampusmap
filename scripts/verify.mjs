...
for(const step of ['What do you want to edit?','Edit','Review and publish','Publish to GitHub & update map']){if(admin.includes(step))ok(`admin wizard "${step}" exists`);else fail(`missing admin wizard element ${step}`)}
for(const type of ['Building','Parking','Shape / area','Words','Appearance','Layers','Advanced JSON']){if(admin.includes(type))ok(`admin editor ${type} exists`);else fail(`missing admin editor ${type}`)}
